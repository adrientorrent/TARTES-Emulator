...
from models import CnnTartesEmulatorLinear
from training import train_loop, evaluate_loop
from utils.norm import CustomNorm


"""Same as main.py, with the model: CnnTartesEmulatorLinear (linear output)"""
...
def main():
    ...
    model_path = f"{data_dir}/model_linear.pt"
    checkpoint = {}
    if RESUME_TRAINING:
    ...
    print("LRScheduler: ExponentialLR(gamma=0.99)")
    if RESUME_TRAINING:
    ...
    csv_file_path = f"{data_dir}/results_linear.csv"
